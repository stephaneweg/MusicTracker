    ...
    internal sealed class Voice
    {
        ...
        public void Start(RegionPair region, int channel, int key, int velocity)
        {
            ...
            if (velocity > 0)
            {
                ...
                // Apply the font's note-on (velocity/key) modulators to Initial Attenuation. When the font
                // supplies its own velocity->attenuation modulator (e.g. MuseScore pianos, for their velocity-
                // layer crossfade), it OVERRIDES the synth's default velocity term; otherwise the default
                // (2 * LinearToDecibels(vel/127), == the SF2 default vel->attenuation modulator) is kept.
                var modAttenuationDb = region.NoteOnAttenuationFromModulators(velocity, key, synthesizer.Channels[channel], out bool hasFontVelAttenuation) / 10F;
                var velocityDb = hasFontVelAttenuation
                    ? -modAttenuationDb
                    : 2 * SoundFontMath.LinearToDecibels(velocity / 127F) - modAttenuationDb;

                var decibels = velocityDb - sampleAttenuation - filterAttenuation;
            ...
            var chan = synthesizer.Channels[channel];
            ...
            // Apply the font's velocity/key -> ModEnvToFilterCutoff modulators (FluidSynth does; MeltySynth otherwise
            // ignores them, leaving the piano's baked negative sweep to mute mid-velocity notes — see RegionPair).
            modEnvToCutoff = (int)MathF.Round(region.ModulationEnvelopeToFilterCutoffFrequencyModulated(velocity, key, chan));
            ...
            instrumentPan = MathF.Clamp(region.Pan, -50F, 50F);
        ...
        public bool Process()
        {
            ...
            if (dynamicCutoff)
            {
                ...
                smoothedCutoff = MathF.Clamp(newCutoff, lowerLimit, upperLimit);
            ...
            currentReverbSend = MathF.Clamp(channelInfo.ReverbSend + instrumentReverb, 0F, 1F);
            currentChorusSend = MathF.Clamp(channelInfo.ChorusSend + instrumentChorus, 0F, 1F);

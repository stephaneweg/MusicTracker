...
namespace MeltySynth
{
    internal sealed class Channel
    {
        ...
        // Raw 7-bit value of every MIDI CC, so SF2 modulators with a CC source (e.g. MuseScore's CC2
        // "single note dynamics" -> attenuation) can read them. CC2 defaults to a mid level so CC2-driven
        // instruments are audible even when the host doesn't send dynamics.
        private readonly byte[] controllers = new byte[128];
        internal const int DefaultBreathController = 100; // CC2 default (mezzo-forte-ish)
        ...
        public void Reset()
        {
            ...
            Array.Clear(controllers, 0, controllers.Length);
            controllers[2] = DefaultBreathController; // CC2
        }

        // Raw CC store, so SF2 modulators can use any MIDI CC (esp. CC2) as a source.
        public void SetController(int index, int value) => controllers[index & 0x7F] = (byte)(value & 0x7F);
        public byte GetController(int index) => controllers[index & 0x7F];

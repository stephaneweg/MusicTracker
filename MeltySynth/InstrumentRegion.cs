...
namespace MeltySynth
{
    ...
    public sealed class InstrumentRegion
    {
        ...
        private readonly Modulator[] modulators = Array.Empty<Modulator>();
        ...
        private InstrumentRegion(Instrument instrument, Zone global, Zone local, SampleHeader[] samples) : this()
        {
            ...
            modulators = CombineModulators(global.Modulators, local.Modulators);
        ...
        // SF2 (2.04 §9.5.1) modulator override: a local (more-specific) modulator with the SAME identity as a
        // global one REPLACES it — modulators are NOT additive within a zone. Identity = source + destination +
        // amount-source + transform (the amount is what differs). Without this dedup, MuseScore_General's pianos —
        // which carry a global velocity->attenuation modulator AND a local one of the same identity (e.g. 800 then
        // 700 cB) — would have both summed, roughly DOUBLING the velocity attenuation. That made the piano ~19 dB
        // too quiet and its velocity curve non-monotonic (each velocity layer duplicated a different amount). The
        // cello has no such duplicate, which is why only keyed instruments like the piano were affected.
        static bool SameIdentity(in Modulator a, in Modulator b) =>
            a.SourceOper == b.SourceOper && a.DestinationOper == b.DestinationOper &&
            a.AmountSourceOper == b.AmountSourceOper && a.TransformOper == b.TransformOper;

        internal static Modulator[] CombineModulators(ArraySegment<Modulator> global, ArraySegment<Modulator> local)
        {
            if (global.Count == 0 && local.Count == 0) return Array.Empty<Modulator>();
            var result = new System.Collections.Generic.List<Modulator>(global.Count + local.Count);
            for (int i = 0; i < global.Count; i++) result.Add(global.Array[global.Offset + i]);
            for (int i = 0; i < local.Count; i++)
            {
                var m = local.Array[local.Offset + i];
                int idx = result.FindIndex(e => SameIdentity(e, m));
                if (idx >= 0) result[idx] = m;   // local overwrites the identical global modulator
                else result.Add(m);
            }
            return result.ToArray();
        }

        internal Modulator[] Modulators => modulators;

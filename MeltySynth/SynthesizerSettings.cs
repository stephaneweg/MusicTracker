...
namespace MeltySynth
{
    ...
    public sealed class SynthesizerSettings
    {
        ...
        internal static int DefaultChannelCount = 16;
        ...
        // The MIDI standard is 16, but nothing in the synthesis is limited to it (voices are pooled by
        // MaximumPolyphony, not per channel), so the host can raise this to size a single synth to a whole
        // project (one channel per track) — keeping the track count effectively unlimited.
        private static void CheckChannelCount(int value)
        {
            if (!(16 <= value && value <= 1024))
            {
                throw new ArgumentOutOfRangeException("The channel count must be between 16 and 1024.");
            }
        }
        ...
        /// <summary>
        /// Gets or sets the number of MIDI channels (16 by default). Raise it to fit one channel per track
        /// so a single synthesizer can render an arbitrarily large project.
        /// </summary>
